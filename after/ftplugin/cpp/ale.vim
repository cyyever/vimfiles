...
if g:gcc_dir!=#''
  ...
  if g:__gcc_exe !=#''
    let b:ale_c_cc_executable= g:__gcc_exe
    let b:ale_cpp_cc_options = s:common_cpp_options
  endif
elseif g:llvm_dir !=#''
  let g:__clang_exe=Executable_any([g:llvm_dir.'/master/bin/clang++'])
  if g:__clang_exe !=#''
    let b:ale_c_cc_executable= g:__clang_exe
    let b:ale_cpp_cc_options = s:common_cpp_options.' -Wno-return-std-move-in-c++11 -isystem'.s:customer_libs_dir.'llvm/master/include/c++ -isystem'.s:customer_libs_dir.'gcc/master/include/c++'
  endif
endif
